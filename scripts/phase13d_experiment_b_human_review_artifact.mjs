...
async function build(payloadPath, outputPath, previewDir) {
  ...
  const table = review.tables.add(`A1:N${rowCount}`, true, "Phase13DExperimentBHumanReview");
  table.style = "TableStyleLight9";
  table.showFilterButton = true;

  instructions.showGridLines = false;
  instructions.getRange("A1:B15").values = [
    ["Phase 13D Experiment B human review", null],
    ["Review every successfully recovered retry answer against only its supplied facts and evidence.", null],
    [null, null],
    ...
    [3, "Do not infer quality from the original mechanical failure; judge the recovered answer itself."],
    [4, "Use notes for unsupported claims, citation mismatch, exact-fact corruption, misleading omissions, refusal errors, or evidence defects."],
    [null, null],
    ...
    ["Review population", "All successfully parsed Experiment B retry answers; no sampling."],
    ["Human verdict status", "Blank — genuine reviewer input required"],
  ];
  instructions.getRange("A1:B15").format.font = { name: "Arial", size: 11, color: "#17212B" };
  ...
  instructions.getRange("A13:A15").format = { fill: "#18242E", font: { name: "Arial", size: 10, bold: true, color: "#FFFFFF" } };
  instructions.getRange("B4:B15").format.wrapText = true;
  instructions.getRange("B13:B15").format.fill = "#F2F4F5";
  instructions.getRange("A1:A15").format.columnWidthPx = 205;
  instructions.getRange("B1:B15").format.columnWidthPx = 760;
  ...
  for (const sheetName of ["Human Review", "Instructions"]) {
    const preview = await workbook.render({ sheetName, range: sheetName === "Human Review" ? `A1:N${rowCount}` : "A1:B15", scale: 1, format: "png" });
...
async function inspect(inputPath, outputPath) {
  ...
  const result = {
    review_values: review.getRange("A1:N200").values,
    instructions_values: instructions.getRange("A1:B20").values,
  };
