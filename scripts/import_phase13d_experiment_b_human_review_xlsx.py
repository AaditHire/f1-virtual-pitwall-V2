"""Validate and explicitly import completed Experiment B recovered-answer review."""
...
from scripts.phase13d_experiment_b_human_review import (
    CANONICAL_REVIEW,
    GROUNDING_VALUES,
    HEADERS,
    MISLEADING_VALUES,
...
def validate(path: Path) -> list[dict]:
    ...
    if not values or values[0] != HEADERS:
        raise ValueError("unexpected Experiment B human-review workbook schema")
    ...
    if len(set(ids)) != len(ids):
        raise ValueError("duplicate Experiment B review ID")
    if ids != expected_ids:
        raise ValueError("Experiment B review population or ordering changed")
    results = []
    protected = []
    ...
    if protected_hash != expected["protected_hash"]:
        raise ValueError("protected Experiment B review packet hash changed")
    return results
...
def main() -> None:
    ...
    print(f"Validated {len(results)} complete Experiment B human-review rows.")
