"""Export the frozen blank review workbook for all recovered Experiment B answers."""
...
def main() -> None:
    ...
    if MANIFEST.exists() or WORKBOOK.exists():
        raise FileExistsError("refusing to replace Experiment B human-review artifact")
    ...
    if len(rows) != payload["review_rows"]:
        raise RuntimeError("exported Experiment B workbook row count changed")
