...
export function Countdown({ target }: { target?: string | null; compact?: boolean }) {
  ...
  const output = `${value.days ? `${String(value.days).padStart(2, "0")}D ` : ""}${String(value.hours).padStart(2, "0")}H ${String(value.minutes).padStart(2, "0")}M ${String(value.seconds).padStart(2, "0")}S`;
